...
## Overview
`cim-bit-serial-pe` is a compact digital Compute-in-Memory (D-CIM) processing element macro slice. It stores stationary multi-bit quantized weights locally while streaming single-bit activation slices per clock cycle, minimizing memory bandwidth and maximizing silicon compute density ($Area / \text{MAC}$).
